...
// Plantilla "Dorado clásico": florales dibujados a mano, dorado + morado.
// La plantilla original del proyecto — sigue siendo una opción seleccionable.
export default function InvitationClasico({ invitation, watermark = false }) {
      ...
      <main>
        <section className="hero-section" id="inicio">
          ...
          <div className="hero-content">
            ...
            <h1>{invitation.eventType}</h1>
            <p className="de">de</p>
            <p className="names">{invitation.names}</p>
            <p className="date">
              {new Intl.DateTimeFormat('es-MX', { day: 'numeric', month: 'long', year: 'numeric' }).format(eventDate)}
            </p>
            <p className="city">
              <MapPin size={17} />
              {invitation.city}
        ...
        <section className="welcome band">
          ...
          <SectionTitle icon={Sparkles} eyebrow="Con cariño" title="Nos llenará de alegría verte" />
        ...
        <section className="locations band">
          ...
          <div className="location-grid">
            <Location place={invitation.church} icon={Church} />
            <Location place={invitation.reception} icon={CalendarHeart} />
          </div>
        </section>
          ...
          <section className="dress-gifts">
            {invitation.dressCode && (
              <article className="info-panel">
                <SectionTitle icon={Shirt} eyebrow="Dress code" title="Cómo vestir" />
                <p>{invitation.dressCode}</p>
              </article>
            )}
            {invitation.giftRegistry && (
              <article className="info-panel">
                <SectionTitle icon={Gift} eyebrow="Mesa de regalos" title="Un detalle, si así lo deseas" />
                <p>Tu presencia es nuestro regalo más especial. Si deseas tener un detalle, puedes verlo aquí.</p>
                <a className="button" href={invitation.giftRegistry} target="_blank" rel="noreferrer">
                  <Gift size={17} />
                  Ver mesa
                </a>
              </article>
        ...
        <section className="photos band">
          <div className="section-flourish right" aria-hidden="true" />
          <SectionTitle icon={Heart} eyebrow="Momentos" title={invitation.names} />
          <div className="photo-grid">
            {photos.map((photo) => (
              <figure className="photo-card" key={`${photo.name}-${photo.src}`}>
        ...
        <section className="rsvp-music">
          <article className="info-panel rsvp">
            ...
            <a className="button" href={rsvpHref} target="_blank" rel="noreferrer">
              <Send size={17} />
